...
\begin{document}
    ...
    \begin{equation}
        \ket{\psi} = \alpha \ket{0} + \beta \ket{1}, \qquad \forall \ \alpha, \beta \in \mathbb{C}
    ...
    Con esta condición, podemos parametrizar el estado del qubit \( \ket{\psi} \) definido en (\ref{eq: combinacion_lineal_estados_1_qubit}) en función de los ángulos \( \theta \) y \( \phi \) como
    
    \begin{equation*}
        \ket{\psi} = \cos \left( \frac{\theta}{2} \right) \ket{0} + e^{i \phi} \sin \left( \frac{\theta}{2} \right), \qquad 0 \leq \theta \leq \pi, \quad 0 \leq \phi \leq 2 \pi
    \end{equation*}

    \vspace{2.5mm}

    Esto nos permite representar el qubit \( \ket{\psi} \) mediante una esfera, la \textit{esfera de Bloch}:
    ...
    De manera que podemos representar todos los posibles estados de un qubit \( \ket{\psi} \) como un vector dentro de esta esfera. Es importante recalcar que, a la hora de medir un qubit, esta medición colapsa sobre el eje \( Z \), es decir, cuando observemos el estado de un qubit al finalizar un algoritmo cuántico, el estado siempre va a colapsar en \( \ket{0} \) o \( \ket{1} \), como ya veremos más adelante. Estos estados son

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\textwidth]{img/bloch_states_0_1.png}
        \caption{Estados \ket{0} \text{y} \ket{1}\text{. Obtenida de [Qiskit]}}\label{fig: bloch_states_0_1}
    \end{figure}
