...
\begin{document}
    ...
    En el contexto de la mecánica cuántica, el \textit{espacio de Hilbert}, el cual denotaremos como \( \mathcal{H} \), es el marco matemático en el que nos apoyamos para explicar el estado físico de las partículas subatómicas, como ya veremos en la sección (\ref{sec: postulados}) explicando los postulados. Este \textit{espacio de Hilbert} no deja de ser un espacio vectorial sobre el cuerpo de los números complejos \( \mathbb{C} \) con un producto escalar hermítico, y que además, este espacio es completo respecto a la norma inducida por dicho producto escalar.

    \vspace{5mm}

    Consideremos un espacio vectorial \( \mathcal{H} = \{ \ket{\varphi}, \ket{x}, \cdots \} \) sobre el cuerpo de los números complejos \( \mathbb{C} \), de manera que se cumplen la propiedad de la suma y la multiplicación por un escalar:
    \begin{align*}
        \ket{\varphi} + \ket{x} \in \mathcal{H}, \qquad &\forall \ket{\varphi}, \ket{x} \in \mathcal{H}\\[10pt]
        \alpha \ket{\varphi} \in \mathcal{H}, \qquad &\forall \ket{\varphi}, \in \mathcal{H} \quad \text{y} \quad \forall \alpha \in \mathbb{C}
    \end{align*}
    ...
    Si \( \mathcal{H} \) satisface estas propiedades, podemos concluir que \( \mathcal{H} \) es un espacio de Hilbert. Con esto podemos definir la norma de un vector \( \varphi \) como
    \vspace{2mm}
    \begin{equation}
        \| \varphi \| = \sqrt{\braket{\varphi | \varphi}} \qquad \in \mathbb{R} \geq 0
        \label{eq: norma_producto_escalar}
    \end{equation}

    \vspace{2mm}

    Podemos observar que \( \| \varphi \| \in \mathbb{R} \geq 0 \), ya que al tratarse de la longitud de un vector, ha de ser un número real no negativo. Con esto podemos introducir la desigualdad de Schwartz, la cual establece lo siguiente:
    \begin{equation}
        \| \braket{x | \varphi} \| ^{2} \leq \| x \| ^{2} \| \varphi \|^{2}
        \label{eq: desigualdad_schwartz}
    \end{equation}

    Al complirse esta desigualdad, el producto interno está bien definido, de donde obtenemos la norma como ya vimos en (\ref{eq: norma_producto_escalar}), pasando a ser un espacio normado cuya norma cumple con la \textit{sucesión de Cauchy}:
    \begin{equation*}
    ...
    Con esto ya podemos llegar a la conclusión de que, un espacio de Hilbert, en resumidas cuentas, es un espacio vectorial \( \mathcal{H} \) con un producto escalar hermítico, y por lo tanto sobre el cuerpo de los números complejos \( \mathbb{C} \). Sabiendo esto podemos trabajar sobre el concepto de ortonormalidad definiendo una base ortonormal de nuestro espacio vectorial \( \mathcal{H} \), como

    \begin{equation*}
        \{ \ket{e_{n}} \} = \{ \ket{1}, \ket{2}, \ldots , \ket{N}\}
    \end{equation*}

    \vspace{2mm}

    Donde \( N \) es la dimensión del espacio vectorial \( \mathcal{H} \). No obstante, esto no deja de ser una base común y corriente. Para que esta base sea ortonormal, se tienen que dar las condiciones de ortonormalidad, donde el producto escalar de dos vectores distintos de dicha base es cero, y la norma de ambos vectores ha de ser igual a la unidad. Esto lo podemos formalizar de la siguiente manera
    \vspace{2mm}
    \begin{equation}
        \braket{n | m} = \delta_{n, m}, \qquad \forall \ket{n}, \ket{m} \in \{ \ket{e_{n}} \}
        \label{eq: ortonormalidad}
    \end{equation}
    \begin{equation}
        \| m \| = 1, \qquad \forall \ket{m} \in \{ \ket{e_{n}} \}
        \label{eq: linealidad}
    \end{equation}
    ...
    Así, el producto escalar de dos vectores de una base ortonormal siempre va a ser cero, y el producto escalar de un vector consigo mismo es la unidad. Entonces, podemos expresar cualquier vector de nuestro espacio vectorial \( \mathcal{H} \) como una combinación lineal de los vectores de la base:
    \begin{equation}
        \ket{\varphi} = \sum_{n = 1}^{N} c_{n} \ket{n}, \qquad \forall \ket{\varphi} \in \mathcal{H}
    ...
    No obstante, a partir de (\ref{eq: ortonormalidad}), podemos deducir el valor de \( c_{n} \) que aparece en (\ref{eq: combinacion_lineal_Hilbert}):
    \begin{align}
        \braket{n | m} &= \sum_{n = 1}^{N} c_{n} \delta_{n, m} = c_{m} \\[10pt]
        c_{m} &= \braket{n | m}, \qquad c_{m} \in \mathbb{C}
    \end{align}

    Entonces, si \( \ket{\varphi} = \sum_{n = 1}^{N} c_{n} \ket{n} \) y \( \ket{x} = \sum_{m = 1}^{N} c_{m} \ket{m} \) son dos vectores cualesquiera de nuestro espacio vectorial \( \mathcal{H} \), podemos deducir que el producto escalar de ambos vectores es
    \begin{equation*}
        \braket{x | \varphi} = \sum_{n, m = 1}^{N} c_{m}^{*} c_{n} \delta_{n, m} = \sum_{n = 1}^{N} c_{m}^{*} c_{n}
    \end{equation*}
    \begin{equation}
        \braket{x | \varphi} = \sum_{n = 1}^{N} c_{m}^{*} c_{n}
        \label{eq: producto_escalar_desarrollado}
    \end{equation}

    De manera que obtenemos un producto, donde los factores provenientes del \textit{bra} (\( \bra{x} \)) son complejos conjugados, pudiéndose definir como una aplicación \( \bra{x} \) sobre un vector \( \varphi \) en un espacio \( \mathcal{H} \), dando como resultado un número complejo:
    \begin{align*}
        \bra{x}: \mathcal{H} &\longrightarrow \mathbb{C} \\[10pt]
        \ket{\varphi} &\longrightarrow \braket{x | \varphi}
    \end{align*}
    
    Donde \( \bra{x} \) se construye con el conjugado hermítico, el cual se define de la siguiente manera:
    \begin{equation*}
        ( \ket{x} )^{\dagger} = \bra{x}
    \end{equation*}
    ...
    \begin{equation}
        \| \varphi \| ^{2} = \braket{\varphi | \varphi} = \sum_{n = 1}^{N} c_{n}^{*} c_{n} = \sum_{n = 1}^{N} \| c_{n} \| ^{2} \geq 0, \qquad \forall \varphi \in \mathcal{H}
    \end{equation}
    ...
    Y debido a que \( \lambda_{j} \neq \lambda_{i} \), obtenemos que ambos vectores \( \psi_{i} \text{,} \psi_{j} \)
    \begin{equation}
