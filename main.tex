...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item Producto escalar hermítico, el cual es una aplicación
            ...
            Con esto ya podemos llegar a la conclusión de que, un espacio de Hilbert, en resumidas cuentas, es un espacio vectorial \( \mathcal{H} \) con un producto escalar hermítico, y por lo tanto sobre el cuerpo de los números complejos \( \mathbb{C} \). Sabiendo esto podemos definir el concepto de ortonormalidad y base ortonormal de un espacio vectorial, en este caso \( \mathcal{H} \), como
            ...
            Donde \( N \) es la dimensión del espacio vectorial \( \mathcal{H} \). Además, se tienen que dar las condiciones de ortonormalidad, donde el producto escalar de dos vectores distintos de dicha base es cero, y la norma de ambos vectores ha de ser igual a la unidad. Esto lo podemos formalizar de la siguiente manera
            \vspace{2mm}
            \begin{equation}
            ...
            Entonces, si \( \ket{\varphi} = \sum_{n = 1}^{N} c_{n} \ket{n} \) y \( \ket{x} = \sum_{m = 1}^{N} c_{m} \ket{m} \) son dos vectores cualesquiera de nuestro espacio vectorial \( \mathbb{H} \), podemos deducir que el producto escalar de ambos vectores es
            \begin{equation}
            ...
            De manera que obtenemos un producto, donde los factores provenientes del \textit{bra} (\( \bra{x} \)) son complejos conjugados. Encontrándonos en este punto, con (\ref{eq: producto_escalar_desarrollado}) podemos desarrollar la norma de un vector en \( \mathcal{H} \), obteniendo que
            \begin{equation}
                \| \varphi \| ^{2} = \braket{\varphi | \varphi} = \sum_{n = 1}^{N} c_{n}^{*} c_{n} = \sum_{n = 1}^{N} \| c_{n} \| ^{2} \geq 0
            \end{equation}
